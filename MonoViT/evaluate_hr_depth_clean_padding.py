...
def evaluate():
    ...
    ######
    file_name = '/home/dan/NeWCRFs/data_splits/val_files_CVPR.txt'
    ...
    with torch.no_grad():
        for image_path, image_name in tqdm(zip(test_image_paths, image_names)):
            ...
            w, h = input_image.size  #(1242,376)
            # print(input_image.size)
            # compute the padding needed to make the width and height divisible by 32
            pad_w = int(math.ceil(w/32))*32 - w
            pad_h = int(math.ceil(h/32))*32 - h

            # pad the image with zeros
            padding = (0, 0, pad_w, pad_h) # left, top, right, bottom
            input_image = transforms.Pad(padding)(input_image) 
            # print(input_image.size) #1248,384
            ...
            disp = outputs[("disp", 0)]
            disp = disp[:, :h+pad_h, :w+pad_w]  # crop to padded size
            disp_resized = disp[:, :, :h, :w]  # crop to original size
            ...
            disp_resized_np = disp_resized.squeeze().cpu().numpy() #(1242,376)
