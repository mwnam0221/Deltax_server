...
def main():
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        if args.save_eval_pngs:
            ...
            if args.dataset == 'nyudepthv2':
                ...
                #change
                pred_d = pred_d.cpu().numpy().squeeze()

                final_disp_pred=final_disp(pred_d)
                pred_depths.append(final_disp_pred)
                
                pred_d = cv2.normalize(pred_d, None, 0, 1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_64F)
                pred_d = 1/pred_d #disparity
                pred_d = (pred_d).astype(np.uint8)
                pred_d = cv2.applyColorMap(pred_d, cv2.COLORMAP_MAGMA)
    ...
    output_path = os.path.join('./results2', "pred.npz")
