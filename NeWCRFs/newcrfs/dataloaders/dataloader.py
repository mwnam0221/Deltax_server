...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            image_path = f"{self.args.data_path}{rgb_file}"
            depth_path = f"{self.args.gt_path}{depth_file}"
...
###########################################################################################################################
            # is_corruption = np.random.random() > 0.6
            # if is_corruption:
            #     corruption_list = ['snow', 'contrast', 'motion_blur', 'zoom_blur','gaussian_noise']
            #     corruption = random.choice(corruption_list)
            #     severity = random.randint(1, 3)

            #     image = corrupt((image*255).astype(np.uint8), corruption_name=corruption, severity=severity)
###########################################################################################################################
...
class ToTensor(object):
    def __init__(self, mode):
        ...
        self.transform = transforms.Resize((352,1120)) #h, w (when kitti data trained)
        # self.transform = transforms.Resize((376,1242)) #h, w (test data)
        # self.transform = transforms.Resize((352,1216)) #h, w (kitti model)
    ...
    def __call__(self, sample):
        ...
        if self.mode == 'test':
            ...
            # return {'image': image, 'U' : self.padding_value[1], 'D' : self.padding_value[3], 'L' : self.padding_value[0], 'R': self.padding_value[2]}#, 'focal': focal}
