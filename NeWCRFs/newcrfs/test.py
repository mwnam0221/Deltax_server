...
def depth_value_to_depth_image1(depth_values, is_gt = False):
    depth_values = cv2.normalize(depth_values, None, 0, 1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_64F)
    disp = 1/depth_values
...
def test(params):
    ...
    with torch.no_grad():
        ...
        for index, sample in enumerate(tqdm(dataloader.data)):
            ...
            resized_pred_depth = cv2.resize(pred_depth, (CFG['W_IMG_SIZE'], CFG['H_IMG_SIZE']), interpolation=cv2.INTER_CUBIC) 
            # print(resized_pred_depth.shape)
...
###############################################################################
                ...
                cv2.imwrite(f'{save_img_dir}/{index}_disp.jpg', depth_value_to_depth_image1(resized_pred_depth))
                final_disp_pred=final_disp(resized_pred_depth)
...
###############################################################################
    ...
    if args.dataset == 'kitti':    
        ...
        # output_path_disp= os.path.join(save_img_dir, "dis_newcrf")
        ...
        output_path_disp= os.path.join(save_img_dir, "depth_newcrf")
        np.save(output_path_disp, pred_depths)
###############################################################################
