...
def eval(model, dataloader_eval, post_process=False):
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        ...
        pred_disps.append(pred_depth[valid_mask])
    ...
    return eval_measures_cpu, pred_disps


def main_worker(args):

    # CRF model
    model = NewCRFDepth_v2(version=args.encoder, inv_depth=False, max_depth=args.max_depth, pretrained=None)
    model.train()
    ...
    model.eval()
    with torch.no_grad():
        eval_measures, pred_disps = eval(model, dataloader_eval, post_process=True)
        # np.savez_compressed('./submission/', data=pred_disps)
